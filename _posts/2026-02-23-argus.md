...
![Test Oracle Problem](https://joyemang33.github.io/assets/img/argus-1.png)
...
<!-- 📸 IMAGE PLACEHOLDER 3
     建议: 插入 paper/幻灯片中的 Figure 1 "Overall pipeline of Argus"，
     这张图展示了 Argus 的 6 个步骤（①生成 schema 和 seed CAQ，
     ②LLM 生成等价 CAQ pair 并用 prover 验证，③生成 SQL snippets corpus，
     ④实例化等价 SQL pairs，⑤实例化数据库，⑥在 DBMS 上验证并报告 bug）。
     这是全文最核心的系统图，务必插入。
-->
...
<!-- 📸 IMAGE PLACEHOLDER 5
     建议: 插入 paper 中的 Table 2 "Argus found bugs statistics"的截图或重制表格，
     显示各 DBMS（Dolt, DuckDB, MySQL, PostgreSQL, TiDB）的 bug 数量、
     状态（Fixed / Confirmed / Duplicate / Pending）和类型（Logic / Other）。
     这是最直接展示成果的表格。
-->
...
36 out of 41 bugs are **logic bugs** — the silent, most dangerous class that cause incorrect query results without any error. Compared to recent works finding 21, 24, 35 logic bugs with manually crafted oracles, Argus finds *more* despite targeting DBMSs already extensively tested.
...
<!-- 📸 IMAGE PLACEHOLDER 8
     建议: 插入 paper 中的 Figure 5 "Cost and efficiency comparison"，
     显示 Argus（Phase I CAQ 生成 + Phase II 实例化）vs 朴素 baseline 在 1 小时内
     累计生成 test case 数量（y 轴 log scale）和累计 LLM 费用（右 y 轴）的对比曲线。
     Argus 在几分钟内完成 Phase I 后，Phase II 的 throughput 远高于 baseline。
-->
...
<table class="table table-sm" style="width: auto; margin: 1rem auto; font-size: 0.88rem; border: 1px solid #ccc;">
  ...
  <tbody style="color: var(--global-text-color);">
    ...
    <tr><td style="border-color: #ccc; padding: 3px 10px;">Metamorphic coverage</td><td style="border-color: #ccc; padding: 3px 10px;">Limited by human creativity</td><td style="border-color: #ccc; padding: 3px 10px;">5–6× higher than state-of-the-art</td></tr>
  </tbody>
</table>
...
2. Develop oracle *prioritization* — given thousands of LLM-generated oracles, which are most likely to find bugs?
